...
- 空间反射与时间反演(**分立对称性**) 
     ...
     **时间反演变换**相当于将时间反号,即$t \to -t$的变换.该变换下电流密度会变号.要保证麦克斯韦方程组在该变换下不变,相应的电磁场应当在时间反演时满足
     $$
...
,则相对原点来说电磁场就具有叫动量密度
$$
\boldsymbol{M}=\boldsymbol{x}\times \boldsymbol{g}=\boldsymbol{x}\times \dfrac{(\boldsymbol{E}\times\boldsymbol{H})}{{c^2}} $$
